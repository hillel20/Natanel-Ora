/* =========================================================
   ...
   (patch complet : anti double-soumission + abort + dédup douce)
   ========================================================= */
   ...
   document.addEventListener('DOMContentLoaded', function () {
    ...
    const SHEET_WEBAPP_URL = 'https://script.google.com/macros/s/AKfycbwYtodzO9zzckH8FL8POB-g32MxvzTrayd8YHE6PsOTOzHUCRht0PAEk9hEkT5IxOwC/exec';
    ...
    /* ====== PATCH ANTI-DOUBLE SOUMISSION (client-side) ====== */
    const INFLIGHT_LOCK_MS = 20000;     // Verrou temporaire 20s (filet de sécurité réseau)
    const DEDUP_TTL_MS     = 60000;     // Même réponse < 60s => ignorée côté client
    let submitting = false;
    let inFlightAbort = null;
    let inflightTimer = null;
  
    function getSubmitBtn(){
      return document.querySelector('.submit-button') || form?.querySelector('button[type="submit"], input[type="submit"]');
    }
    ...
    function lockUI() {
      ...
      if (btn) {
        ...
        btn.setAttribute('data-original-text', isInput ? (btn.value||'') : (btn.textContent||''));
        if (isInput) btn.value = 'Envoi…'; else btn.textContent = 'Envoi…';
        btn.disabled = true;
        btn.setAttribute('aria-disabled', 'true');
      }
      clearTimeout(inflightTimer);
      inflightTimer = setTimeout(() => { if (submitting) unlockUI(); }, INFLIGHT_LOCK_MS);
    }
    function unlockUI() {
      submitting = false;
    ...
    function buildDedupKey() {
      const first = (document.querySelector('input[name="first_name"]')?.value||'').trim().toLowerCase();
      const last  = (document.querySelector('input[name="last_name"]')?.value||'').trim().toLowerCase();
      const tel   = (document.querySelector('input[name="tel"]')?.value||'').trim().toLowerCase();
      const presence = presenceSelect?.value || '';
      const come = preferenceSelect?.value || '';
      return [first,last,tel,presence,come].join('|');
    }
    function isRecentDuplicate(key) {
      const raw = sessionStorage.getItem('rsvp_last_submit');
      if (!raw) return false;
      try {
        const obj = JSON.parse(raw);
        return obj.key === key && (Date.now() - obj.ts) < DEDUP_TTL_MS;
      } catch { return false; }
    }
    function rememberSubmit(key) {
      sessionStorage.setItem('rsvp_last_submit', JSON.stringify({ key, ts: Date.now() }));
    }
  
    // Antidouble-clic / Enter quand en cours
    document.addEventListener('keydown', (e) => {
      if (!form) return;
      if ((e.key === 'Enter' || e.keyCode === 13) && submitting) { e.preventDefault(); e.stopPropagation(); }
    }, true);
    getSubmitBtn()?.addEventListener('dblclick', (e)=>{ if (submitting){ e.preventDefault(); e.stopPropagation(); }}, true);
    getSubmitBtn()?.addEventListener('click', (e)=>{ if (submitting){ e.preventDefault(); e.stopPropagation(); }}, true);
  
    /* ---------- Lock définitif existant ---------- */
    function lockForm(){
      sessionStorage.setItem(SUBMIT_LOCK_KEY,'1');
      const btn = getSubmitBtn();
      if (btn) { btn.disabled = true; btn.style.opacity = '0.6'; }
      if (form) [...form.elements].forEach(x=>x.disabled = true);
    }
    (function initLock(){
    ...
    form?.addEventListener('submit', async (e) => {
      ...
      // Sécurité client: nonce + timestamp (utile si GAS filtre un jour)
      try { payload.client_nonce = crypto.randomUUID(); } catch { payload.client_nonce = String(Math.random()); }
      payload.client_ts = String(Date.now());
  
      // Prépare l’URL-encoded (comme ton implémentation)
      ...
      try {
        ...
        if (res.ok && j.ok !== false) {
          rememberSubmit(dedupKey);
          if (okMsg) okMsg.style.display = 'block';
          unlockUI();   // réactive pour laisser un autre invité répondre sur le même device
          lockForm();   // garde ta protection “1 seule fois”
          return;
        }
        throw new Error(`HTTP ${res.status} ${text}`);
      } catch (err) {
        console.error('Erreur fetch vers Google Script:', err);
        unlockUI(); // on rend la main
        ...
        for (const [k,v] of Object.entries(payload)) {
          if (k==='kids' || k==='kids_count' || k==='lang' || k==='user_agent' || k==='client_nonce' || k==='client_ts') continue;
